...
public sealed class BankCentcomConsoleBoundUserInterface : BoundUserInterface
{
    ...
    protected override void Open()
    {
        ...
        _window.OnCreateAccount +=
            (station, name, bal) => SendMessage(new CentcomCreateAccountMessage(station, name, bal));
        _window.OnResetPin += (station, acc) => SendMessage(new CentcomResetPinMessage(station, acc));

        _window.OnIssueFine += (station,
                targetType,
                targetId,
                amount,
                reason,
                ...
                announce,
                customAnnouncement) =>
            SendMessage(new CentcomIssueFineMessage(station,
                targetType,
                targetId,
                amount,
                reason,
        ...
        _window.OnEditAccount += (station, targetId, bal, del, isDept) =>
            SendMessage(new CentcomEditAccountMessage(station, targetId, bal, del, isDept));
