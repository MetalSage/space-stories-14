...
public sealed partial class BankCentcomConsoleSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        Subs.BuiEvents<BankCentcomConsoleComponent>(BankCentcomConsoleUiKey.Key,
            subs =>
            {
    ...
    private void UpdateUi(EntityUid uid)
    {
        ...
        if (!TryComp<BankCentcomConsoleComponent>(uid, out var comp))
            return;

        var selectedStation = comp.SelectedStation;
        if (selectedStation == null || !Exists(selectedStation.Value) ||
            !HasComp<StationBankComponent>(selectedStation.Value))
        {
            if (stations.Count > 0)
        ...
        if (selectedStation != null && Exists(selectedStation.Value))
        {
            if (TryComp<StationFinancialLogComponent>(selectedStation.Value, out var logComp))
            {
                foreach (var log in logComp.Logs)
                {
                    ...
                    {
                        Timestamp = log.Timestamp, Source = log.Source, Destination = log.Destination,
                        Amount = log.Amount, Reason = log.Reason,
                    });
                }
            }
            ...
            {
                string name = acc.Key;
                if (_prototypeManager.TryIndex(acc.Key, out var cargoAcc))
        ...
        _ui.SetUiState(uid,
            BankCentcomConsoleUiKey.Key,
            new BankCentcomConsoleState(stations,
                selectedStation != null ? GetNetEntity(selectedStation.Value) : null,
                logs,
                accounts,
                salMod,
                salFreq));
    ...
    private void OnCreateAccount(EntityUid uid, BankCentcomConsoleComponent component, CentcomCreateAccountMessage args)
    {
        ...
        if (TryComp<StationBankComponent>(station, out var bank))
        {
            ...
            _popup.PopupEntity(Loc.GetString("stories-bank-centcom-account-created-popup",
                    ("name", args.OwnerName),
                    ("number", accNum),
                    ("pin", pin)),
                uid,
    ...
    private void OnResetPin(EntityUid uid, BankCentcomConsoleComponent component, CentcomResetPinMessage args)
    {
        ...
        if (TryComp<StationBankComponent>(station, out var bank) &&
            bank.Accounts.TryGetValue(args.TargetId, out var acc))
        {
            acc.Pin = _bank.GeneratePin();
            Dirty(station, bank);

            _popup.PopupEntity(Loc.GetString("stories-bank-centcom-pin-reset-popup",
    ...
    private void OnIssueFine(EntityUid uid, BankCentcomConsoleComponent component, CentcomIssueFineMessage args)
    {
        ...
        var totalFined = 0;
        var targetAnnounceName = Loc.GetString("stories-bank-centcom-target-all-name");

        void ApplyFine(string accId, bool isDepartment)
        {
            var fineAmount = args.Amount;
                    ...
                    if (args.SendNotification && _bank.TryGetMindByAccountNumber(accId, out var mindId))
                    {
                        if (args.IsPercentage)
                        {
                            _economy.TrySendNotification(mindId,
                                ...
                                Loc.GetString("stories-bank-app-notification-fine-percent-body",
                                    ("percent", args.Amount),
                                    ("amount", fineAmount),
                                    ("reason", args.Reason)));
                        }
        ...
        if (args.TargetType == CentcomFineTarget.Crew)
        {
            ...
            targetAnnounceName =
                _bank.TryGetAccount(station, args.TargetId, out var acc) ? acc.OwnerName : args.TargetId;
        }
        else if (args.TargetType == CentcomFineTarget.Department)
        {
            ApplyFine(args.TargetId, true);
            targetAnnounceName = _prototypeManager.TryIndex<CargoAccountPrototype>(args.TargetId, out var cargoAcc)
                ? Loc.GetString(cargoAcc.Name)
                : args.TargetId;
        }
        ...
        {
            var announcementBody = args.CustomAnnouncement ??
                                   Loc.GetString("stories-bank-centcom-announcement-fine-complex");

            var fineValueStr = args.IsPercentage ? $"{args.Amount}%" : $"{args.Amount} кр.";
    ...
    private void OnEditAccount(EntityUid uid, BankCentcomConsoleComponent component, CentcomEditAccountMessage args)
    {
        ...
        if (args.Delete)
        {
            if (!args.IsDepartment && TryComp<StationBankComponent>(station, out var bank) &&
                bank.Accounts.Remove(args.TargetId))
            {
                Dirty(station, bank);
                if (_bank.TryGetMindByAccountNumber(args.TargetId, out var mindId))
                {
                    _economy.TrySendNotification(mindId,
                        ...
                        Loc.GetString("stories-bank-app-notification-account-deleted"));
                }
        ...
        else
        {
            if (args.IsDepartment)
            {
                ...
                if (_cargoSystem.TrySetBankAccount(station, proto, args.NewBalance))
                {
                    _popup.PopupEntity(Loc.GetString("stories-bank-centcom-edit-success"), uid, actor);
                    UpdateUi(uid);
                    return;
            ...
            else
            {
                ...
                {
                    var oldBal = bank.Accounts[args.TargetId].Balance;
                    ...
                    if (_bank.TryGetMindByAccountNumber(args.TargetId, out var mindId))
                    {
                        _economy.TrySendNotification(mindId,
                            Loc.GetString("stories-bank-app-notification-admin-change-title"),
                            Loc.GetString("stories-bank-app-notification-admin-change-body",
                                ("old", oldBal),
                                ("new", args.NewBalance)));
                    }
