...
using Content.Shared._Stories.Pulling;
...
public sealed partial class StoriesFiremanCarrySystem : EntitySystem
{
    ...
    [Dependency] private StoriesPullingEffectSystem _pullingEffect = default!;
    [Dependency] private StandingStateSystem _standing = default!;
    [Dependency] private StoriesSpriteSystem _storiesSprite = default!;
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<CanFiremanCarryComponent, StoriesFiremanCarryPullToggleEvent>(OnCarrierPullToggle);
    ...
    private void OnCarriableCanDropDragged(Entity<FiremanCarriableComponent> ent, ref CanDropDraggedEvent args)
    {
        ...
        if (IsPulling(args.User, ent.Owner))
        {
            args.CanDrop = true;
            args.Handled = true;
        }
    ...
    private void OnCarrierCanDropTarget(Entity<CanFiremanCarryComponent> ent, ref CanDropTargetEvent args)
    {
        ...
        if (IsPulling(args.User, args.Dragged))
        {
            args.CanDrop = true;
            args.Handled = true;
        }
    ...
    private void OnCarrierBeforeThrow(Entity<CanFiremanCarryComponent> carrier, ref BeforeThrowEvent args)
    {
        ...
        args.Direction *= carriable.ThrowDistanceModifier;
        args.ThrowSpeed *= carriable.ThrowDistanceModifier;
    }
    ...
    private bool ValidateThrow(Entity<CanFiremanCarryComponent> carrier, EntityUid target, FiremanCarriableComponent carriable, EntityUid heldItem)
    {
        ...
        {
            _popup.PopupClient(Loc.GetString("stories-fireman-carry-need-carry-first"), carrier, carrier, PopupType.SmallCaution);
            return false;
        }

        if (!carriable.CanThrow)
        {
            _popup.PopupClient(Loc.GetString("stories-fireman-carry-cant-throw"), carrier, carrier, PopupType.SmallCaution);
            return false;
        }
    ...
    private void OnCarriableDragDropDragged(Entity<FiremanCarriableComponent> ent, ref DragDropDraggedEvent args)
    {
        ...
        {
            _popup.PopupClient(Loc.GetString("stories-fireman-carry-need-aggressive-grab"), ent, user, PopupType.MediumCaution);
            return;
        }
        ...
        _popup.PopupClient(Loc.GetString("stories-fireman-carry-start-loading", ("targetName", target)), ent, user, PopupType.Medium);
    }
    ...
    {
        return ValidateCarrierWhitelist(user, target, carriable);
    }
    ...
    {
        if (!IsPulling(args.DoAfter.Args.User, ent.Owner))
            args.Cancel();
    }
    ...
    private void OnCarriableMoveInput(Entity<FiremanCarriableComponent> ent, ref MoveInputEvent args)
    {
        ...
        var doAfter = new DoAfterArgs(EntityManager, ent, ent.Comp.BreakDelay, ev, ent);
        if (!_doAfter.TryStartDoAfter(doAfter))
            return;

        ent.Comp.BreakingFree = true;
        if (!IsBeingPulled(ent.Owner, out var puller))
            return;

        var selfMsg = Loc.GetString("stories-pull-break-start-self", ("puller", puller));
        ...
        foreach (var other in others.Recipients)
        {
            ...
            var msg = Loc.GetString("stories-pull-break-start-others", ("puller", pullerName), ("pulled", pulledName));
    ...
    private void OnCarriableBreakCarryDoAfter(Entity<FiremanCarriableComponent> ent, ref BreakFiremanCarryDoAfterEvent args)
    {
        ...
        var selfMsg = Loc.GetString("stories-pull-break-finish-self", ("puller", puller));
        _popup.PopupClient(selfMsg, ent, ent, PopupType.MediumCaution);
        ...
        foreach (var other in others.Recipients)
        {
            ...
            var pullerName = Identity.Name(puller, EntityManager, recipient);
            var pulledName = Identity.Name(ent, EntityManager, recipient);
            var msg = Loc.GetString("stories-pull-break-finish-others", ("puller", pullerName), ("pulled", pulledName));
            _popup.PopupEntity(msg, ent, recipient, PopupType.MediumCaution);
        }
    }
    ...
    private void OnCarrierPullToggle(Entity<CanFiremanCarryComponent> ent, ref StoriesFiremanCarryPullToggleEvent args)
    {
        args.Handled = true;
        ...
        _standing.Down(pulling, force: true);

        _pullingEffect.PlayPullEffect(ent, pulling);

        var selfMsg = Loc.GetString("stories-pull-aggressive-self", ("pulled", pulling));
        ...
        foreach (var other in others.Recipients)
        {
            ...
            var msg = Loc.GetString("stories-pull-aggressive-others", ("puller", pullerName), ("pulled", pulledName));
    ...
    private bool ValidateAggressiveGrab(Entity<CanFiremanCarryComponent> carrier, EntityUid target)
    {
        if (TryComp(target, out FiremanCarriableComponent? carriable))
        {
            ...
            {
                _popup.PopupClient(Loc.GetString("stories-fireman-carry-cant-grab-aggressively"), target, carrier, PopupType.MediumCaution);
                return false;
            }
        }
    ...
    private void StopCarry(Entity<CanFiremanCarryComponent?> user, Entity<FiremanCarriableComponent?>? targetNullable)
    {
        ...
        if (Resolve(user, ref user.Comp, false))
        {
            ...
            _storiesSprite.SetRenderOrder(user, 0);
    ...
    {
        return IsBeingPulled(target, out var user) &&
               TryComp(user, out CanFiremanCarryComponent? carrier) &&
               carrier.AggressiveGrab;
    }

    public bool IsPulling(EntityUid puller, EntityUid target)
    {
        return _pullerQuery.TryComp(puller, out var pullerComp) && pullerComp.Pulling == target;
    }

    public bool IsBeingPulled(EntityUid target, out EntityUid user)
    {
        user = default;
        if (!_pullableQuery.TryComp(target, out var pullableComp) || pullableComp.Puller == null)
            return false;

        user = pullableComp.Puller.Value;
        return pullableComp.BeingPulled;
    }
