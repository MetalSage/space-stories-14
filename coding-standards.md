...
* Значения, используемые в коде систем, должны определяться через YAML компонента или, как минимум, через CVar, определенный в `SCCVars`.
...
* Директивы `using` являются исключением для комментирования: их комментировать не нужно, и не надо оставлять там комментарии о принадлежности к форку.
